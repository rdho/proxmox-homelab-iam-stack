...
#                4. Installs and configures cloudflared tunnel for auth.sampledomain.com
...
#   - DNS: auth.sampledomain.com and iam.sampledomain.com are set up in Cloudflare
...
# === STEP 9: ADD INGRESS CONFIG FOR auth.sampledomain.com ===
...
if [[ -f /etc/cloudflared/config.yml ]]; then
  ...
  if ! grep -q "auth.sampledomain.com" /etc/cloudflared/config.yml; then
...
echo ""
echo "  This creates the CNAME: auth.sampledomain.com → <UUID>.cfargotunnel.com"
...
# Check 6: iam.sampledomain.com accessible on port 443 from LAN
